defmodule Dynamo do
  ...
  * The second element is the updated state.
  ...
  defp reset_election_timer(state) do
    ...
    new_timer = Emulation.timer(get_election_time(state), :election_timer)
  ...
  defp reset_heartbeat_timer(state) do
    ...
    new_timer = Emulation.timer(state.heartbeat_timeout, :heartbeat_timer)
    save_heartbeat_timer(state, new_timer)
  end
    ...
    receive do
      ...
      {sender,
       %Raft.AppendEntryRequest{
        ...
        # follower
        IO.puts(
          "#{inspect(whoami())} Follower Received append entry for term #{term} with leader #{leader_id} " <>
            "(#{leader_commit_index})"
        )
        ...
        # AppendEntries Req from expired leader
        if term < state.current_term do
          response = %Raft.AppendEntryResponse{
          ...
          case get_log_entry(state, prev_log_index) do
            ...
            # follower log not in-sync with leader
            log_entry when log_entry.term != prev_log_term ->
            ...
            _ ->
              ...
              if state.commit_index > state.last_applied do
                ...
                {_, state} = commit_log_index(state, state.last_applied)
                follower(state, extra_state)
              end
  ...
  def become_leader(state) do
    ...
    IO.puts(" #{inspect(whoami())} becoming leader ")
    state = update_leader(state, whoami())
    #initial heartbeat
    request = %Raft.AppendEntryRequest {
      term: state.current_term,
    ...
    broadcast_to_others(state, request)


    if state.election_timer != nil do
      Emulation.cancel_timer(state.election_timer)
    end
    state = %{ state | election_timer: nil, voted_for: nil }

    #start heartbeat timer
    state = reset_heartbeat_timer(state)

    leader(make_leader(state), Map.new())
    ...
    receive do
      ...
      {sender,
       %Raft.AppendEntryRequest{
        ...
        IO.puts(
          "#{inspect(whoami())} Leader Received append entry for term #{term} with leader #{
            leader_id
          } " <>
            "(#{leader_commit_index})"
        )
      ...
      {sender,
       %Raft.AppendEntryResponse{
        ...
        case succ do
          true ->
            log_entry_to_commit = get_log_entry(state, index)
            extra_state_count = Map.get(extra_state, log_entry_to_commit.requester) + 1
            extra_state = Map.put(extra_state, log_entry_to_commit.requester, extra_state_count)
            
            state = %{state | next_index: Map.put(state.next_index, sender, index+1)}
            # TODO update match_index

            #if majority responses received commit and respond to client
            if extra_state_count >= 1 + div(length(state.view),2) do
              if log_entry_to_commit.term == state.current_term and index > state.commit_index do
                {{addr, stat}, etats} = commit_log_entry(state, log_entry_to_commit)
                
                # IO.puts("#{inspect(state.log)} SEND TO CLIENT: #{log_entry_to_commit.requester}, addr: #{addr}, stat: #{stat}")
                
                send(addr, stat)

                if etats.commit_index > etats.last_applied do
                  etats = %{etats | last_applied: etats.last_applied+1}
                end

                leader(etats, extra_state)
              end 
            end
          ...
          false ->
            #convert to follower if term from APC response > current_term
            if term > state.current_term do
              Emulation.cancel_timer(state.heartbeat_timer)
              state = %{state | current_term: term, heartbeat_timer: nil}
              become_follower(state)
            end
            if index >= Map.get(state.next_index, sender) do
              next_ind = Map.get(state.next_index, sender)
              state = %{ state | next_index: Map.put(state.next_index, sender, next_ind-1) }
              new_next_ind = Map.get(state.next_index, sender)
              entr = get_log_suffix(state, new_next_ind)
              prev_log_entry = get_log_entry(state, new_next_ind-1)
              prev_log_t = prev_log_entry.term
              request = %Raft.AppendEntryRequest {
                term: state.current_term,
                leader_id: state.current_leader,
                prev_log_index: new_next_ind-1,
                prev_log_term: prev_log_t,
                entries: entr,
                leader_commit_index: state.commit_index
              } 
              send(sender, request)
      ...
      {sender, :nop} ->
        ...
        leader(state, Map.put(extra_state, sender, 1))
    ...
    receive do
      ...
      {sender,
       %Raft.AppendEntryRequest{
        ...
        IO.puts(
          "#{inspect(whoami())} Candidate received append entry for term #{term} " <>
            "with leader #{leader_id} " <>
            "(#{leader_commit_index})"
        )

        # AppendEntry req from previous term
        if term < state.current_term do
          response = %Raft.AppendEntryResponse{
            term: state.current_term, 
            log_index: get_last_log_index(state), 
      ...
      {sender,
       %Raft.RequestVoteResponse{
        ...
        if term == state.current_term and granted == true do
          ...
          if votes >= 1 + div(length(state.view),2) do
            IO.puts(
              "Candidate #{inspect(whoami())} becomes leader in term #{inspect(term)}"
            )
            become_leader(state)
          end
